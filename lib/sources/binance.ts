...
const KLINES_URL = 'https://fapi.binance.com/fapi/v1/klines?symbol=SKHYUSDT&interval=1d&limit=100';
...
export async function fetchBinanceQuote(): Promise<Quote> {
  const json = await fetchJson(TICKER_URL, { label: 'binance ticker' });
  return parseBinanceQuote(json);
}
...
export async function fetchBinanceDaily(): Promise<DailyPoint[]> {
  const json = await fetchJson(KLINES_URL, { label: 'binance klines' });
  return parseBinanceDaily(json);
}
